...
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'package:geolocator/geolocator.dart';
import 'package:location_tracking_app/location_history.dart';
...
class _GoogleMapScreenState extends State<GoogleMapScreen> {
  ...
  MapType _currentMapType = MapType.normal;
  List<LatLng> _trackedPositions = [];
  List<Map<String, dynamic>> _travelHistory = [];
  List<Marker> _markers = [];
  double _totalDistance = 0.0;
  bool _isTracking = false;
  ...
  void _addMarker(Position position) {
    _markers.add(
      Marker(
        markerId: MarkerId('current_location_${position.timestamp}'),
        position: LatLng(position.latitude, position.longitude),
        icon: BitmapDescriptor.defaultMarkerWithHue(
            _isTracking ? BitmapDescriptor.hueBlue : BitmapDescriptor.hueRed),
      ),
    ...
    if (_isTracking) {
      setState(() {
        ...
        _travelHistory.add({
          ...
          'duration': DateTime.now().difference(_trackingStartTime!),
          ...
          child: FloatingActionButton(
            ...
            onPressed: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      LocationHistoryScreen(travelHistory: _travelHistory),
                ),
              );
            },
          ),
